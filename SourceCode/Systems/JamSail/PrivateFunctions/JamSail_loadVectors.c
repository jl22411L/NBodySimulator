...
int JamSail_loadVectors(double         *p_stateEstimateVector_out,
                        ...
                        JamSail_Params *p_jamSail_params_in)
{
  ...
  for (i = 0; i < 3; i++)
  {
    ...
    *(p_measurementEstimateVector_out + i + 3) =
        p_jamSail_state_in->magneticFieldEstimate_Bod[i];
  ...
  for (i = 0; i < 3; i++)
  {
    ...
    *(p_measurementSensorVector_out + i + 3) = filteredMagneticField_Bod_nT[i];
  ...
  /* Normalise vectors */
  GMath_findUnitVector((p_measurementSensorVector_out + 3),
                       (p_measurementSensorVector_out + 3));
  GMath_findUnitVector((p_measurementSensorVector_out + 6),
                       (p_measurementSensorVector_out + 6));
  GMath_findUnitVector((p_measurementEstimateVector_out + 3),
                       (p_measurementEstimateVector_out + 3));
  GMath_findUnitVector((p_measurementEstimateVector_out + 6),
                       (p_measurementEstimateVector_out + 6));
