    ...
    int     nDegreeMax_in)
{
  ...
  /* Clear array  */
  GZero(p_legPolyArray_out, double[nDegreeMax_in + 1][nDegreeMax_in + 1]);

  /* Set initial values */
  *(p_legPolyArray_out + (nDegreeMax_in + 1) * 0 + 0) = 1;
  *(p_legPolyArray_out + (nDegreeMax_in + 1) * 1 + 0) = inputValue_in;

  /* Fill first column in output array */
  for (n = 2; n <= nDegreeMax_in; n++)
  {
    *(p_legPolyArray_out + (nDegreeMax_in + 1) * n + 0) =
        (((double)(2 * n - 1)) * inputValue_in *
             *(p_legPolyArray_out + (nDegreeMax_in + 1) * (n - 1) + 0) -
         ((double)(n - 1)) *
             *(p_legPolyArray_out + (nDegreeMax_in + 1) * (n - 2) + 0)) /
        ((double)n);
  }
  ...
  {
    /* Find the diagnol */
    *(p_legPolyArray_out + (nDegreeMax_in + 1) * m + m) =
        -((double)(2 * m - 1)) * sqrt(1 - inputValue_in * inputValue_in) *
        *(p_legPolyArray_out + (nDegreeMax_in + 1) * (m - 1) + (m - 1));

    /* Find the value below the diagnol */
    *(p_legPolyArray_out + (nDegreeMax_in + 1) * (m + 1) + (m)) =
    ...
    {
      *(p_legPolyArray_out + (nDegreeMax_in + 1) * n + m) =
          (((double)(2 * n - 1)) * inputValue_in *
               *(p_legPolyArray_out + (nDegreeMax_in + 1) * (n - 1) + m) -
           ((double)(n + m - 1)) *
               *(p_legPolyArray_out + (nDegreeMax_in + 1) * (n - 2) + m)) /
          ((double)(n - m));
    }
  }
